...
export default function BoissonsScreen({navigation,route}) {
    ...
    return <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <FlatList data={All.Boissons} renderItem={({ item, index }) => {
            ...
            {/* <View style={{flex:1,alignItems:'center',justifyContent:'space-between',margin:17,flexDirection:"row",height:'0%'}}>
                {/* <View style={{flexDirection:'column',justifyContent:'space-between',height:"100%",width:"100%",alignItems:'flex-start',margin:15}}> */}
                    {/* <TouchableOpacity onPress={()=>{
                        let x={...All}
                        x.Boissons[index].isSmall=true
                        setAll(x)
                    }}>
                        <Text style={{fontWeight:'bold',fontSize:18,textDecorationLine:Boissons[index].isSmall&&Boissons[index].quantity>0?'underline':'none',color:(Boissons[index].isSmall&&Boissons[index].quantity>0)?'black':'lightgray'}}>Petite</Text>
                        <Text style={{fontSize:11,textAlign:'center',color:(Boissons[index].isSmall&&Boissons[index].quantity>0)?'black':'lightgray'}}>{item.unitPrice[0]}</Text>
                    </TouchableOpacity >
                    <TouchableOpacity onPress={()=>{
                        let x={...All}
                        x.Boissons[index].isSmall=false
                        setAll(x)
                    }} >
                        <Text style={{fontWeight:'bold',fontSize:18,textDecorationLine:!Boissons[index].isSmall&&Boissons[index].quantity>0?'underline':'none',color:(!Boissons[index].isSmall&&Boissons[index].quantity>0)?'black':'lightgray'}}>Moyenne</Text>
                        <Text style={{fontSize:11,textAlign:'center',color:(!Boissons[index].isSmall&&Boissons[index].quantity>0)?'black':'lightgray'}}>{item.unitPrice[1]}</Text>
                    </TouchableOpacity> */}
                {/* </View> */}
        ...
        {index===All.Boissons.length-1?<View style={{height:60}}></View>:null}
        </View>
        }} />
        <NativeBaseProvider>
